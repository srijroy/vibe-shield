 ...
 */
async function scanFile(document, context) {
    ...
    try {
        ...
        if (result.total_findings > 0) {
            ...
            vscode.window.showWarningMessage(`VibeShield found ${result.total_findings} potential secret(s) in ${path.basename(document.fileName)}`, 'View Details', 'Fix All').then(selection => {
...
function callGoBridge(filePath, context) {
    return new Promise((resolve, reject) => {
        ...
        // Get configuration
        const config = vscode.workspace.getConfiguration('vibeshield');
        const entropyThreshold = config.get('entropyThreshold', 3.5);
        // Build args
        const args = [filePath];
        ...
        const child = (0, child_process_1.spawn)(bridgePath, args);
        let stdout = '';
        let stderr = '';
...
function updateDiagnostics(document, result) {
    const diagnostics = result.findings.map(finding => {
        ...
        const diagnostic = new vscode.Diagnostic(range, `Potential ${finding.type} API key detected (entropy: ${finding.entropy.toFixed(2)}, confidence: ${finding.confidence})`, vscode.DiagnosticSeverity.Warning);
 ...
 */
async function fixSecrets(document, context) {
    ...
    const answer = await vscode.window.showWarningMessage(`This will move ${result.total_findings} secret(s) to .env and update your code. Continue?`, 'Yes', 'No');
    if (answer !== 'Yes') {
        return;
    }
